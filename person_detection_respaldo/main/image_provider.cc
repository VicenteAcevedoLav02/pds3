...
TfLiteStatus GetImage(int image_width, int image_height, int channels, int8_t* image_data) {
  ...
  /* for (int i = 0; i < image_height; i++) {
    for (int j = 0; j < image_width; j++) {
      int8_t pixel_value = image_data[i * image_width + j];
      // Convertir el valor a un rango adecuado para la consola
      // Los valores de los píxeles en escala de grises deben estar entre 0 y 255
      uint8_t gray_value = static_cast<uint8_t>(pixel_value + 128); // Ajusta el rango
      std::cout << (gray_value > 127 ? '#' : ' '); // Usa '#' para valores altos y espacio para bajos
    }
